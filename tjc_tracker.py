    ...
    def walk(lst):
        for item in (lst or []):
            if not isinstance(item, dict):
                continue
            sku = str(item.get("stockCode") or "")
            if sku and sku not in out:
                price = item.get("currentPrice")
                if price is None:
                    price = item.get("price")
                stock = item.get("stockRemaining")
                if stock is None:
                    stock = item.get("quantity")
                title = item.get("itemDescription") or item.get("itemName")
                out[sku] = {
                    "sku": sku,
                    "auctionCode": item.get("auctionCode"),
                    "title": title,
        ...
        for auc in (hour.get("auctions") or []):
            ...
            for v in flatten_auction_family([auc]):
                ...
                if not vsku or vsku == sku:
                    continue
                vrec = {"title": v["title"] or title, "price": v["price"], "auctionCode": v["auctionCode"], "img": img, "productUrl": product_url}
                if vsku not in by_sku:
                    by_sku[vsku] = vrec
                if v["auctionCode"] and v["auctionCode"] not in by_auction:
                    by_auction[v["auctionCode"]] = {"sku": vsku, **vrec}
    ...
    if r and r["sku"]:
        ...
        for v in variants:
            ...
            if not vsku or vsku == r["sku"]:
                continue
            va = air.setdefault(vsku, {"sku": vsku, "firstAir": t, "tvPrice": "", "remarks": ""})
            va.update({
                "lastAir": t, "title": v["title"] or va.get("title"),
                "pdpPrice": v["price"], "auctionCode": v["auctionCode"],
                "buy": True, "login": False,
                "date": now_date, "time": now_str,
                "productUrl": r["productUrl"] or va.get("productUrl"),
                "lastUpdated": t, "source": "gha-variant",
            })
        if variants:
